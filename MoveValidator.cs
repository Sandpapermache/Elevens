...
public class MoveValidator
{
    ...
    {
        for (int i = 0; i < tableCards.Count; i++)               //Goes through table list to check if any legal moves are available
        {
        ...
        return values.Contains(11) && values.Contains(12) && values.Contains(13);   //There's is a J,Q,K
    }

    private bool IsValidPair(Card a, Card b)
    {
        return (a.getValue() + b.getValue() == 11);         // Returns if card value adds to 11
    }

    private bool IsValidTriple(IReadOnlyList<Card> three)
    {
        var values = three.Select(c => c.getValue()).ToList();
        return values.Contains(11) && values.Contains(12) && values.Contains(13);       //Returns if there is a J,Q,K based on their respective value
    }
}